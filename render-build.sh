...
echo "[build] Installing OS packages required for pyodbc and SQL Server ODBC"
...
echo "[build] Configuring Microsoft package repository for ODBC Driver 18"
. /etc/os-release
if [[ "${ID:-}" == "debian" ]]; then
...
echo "[build] Verifying installed ODBC drivers"
if command -v odbcinst >/dev/null 2>&1; then
  odbcinst -q -d || true
else
  echo "[build] odbcinst command not available"
fi
